    ...
    public partial class CashIn_Form : Form
    {
        ...
        private void updateTotalUSDBalance(double newBalance)
        {
            ...
            try
            {
                ...
                string query = "UPDATE Balance SET [USDBalance] = @newBalance, [USD_Last_Modified_Date] = @newDate WHERE ID = 1";
                command.CommandText = query;
                command.Parameters.Add("@newBalance", OleDbType.Double).Value = newBalance;
                command.Parameters.Add("@newDate", OleDbType.DBDate).Value = DateTime.Now;
                int i = command.ExecuteNonQuery();
        ...
        private void refreshGridOfToday()
        {
            ...
            try
            {
                string query = "select [ID],[Transaction_Date], [Transaction_Time], [Quantity],[Cash_Currency],[Notes] from Cash_Transaction where [Transaction_Type] = 'IN'"
        ...
        private void CashIn_Form_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                // string query = "select * from Cash_Transaction where [Transaction_Date]=Date()";// + " and [Transaction_Type] = 'IN' ";
                string query = "select [ID],[Transaction_Date], [Transaction_Time], [Quantity],[Cash_Currency],[Notes] from Cash_Transaction where [Transaction_Type] = 'IN'"
                  + " and [Transaction_Date]=Date() order by ID DESC";
                OleDbCommand command = connection.CreateCommand();
                command.CommandType = CommandType.Text;
        ...
        private void save_cash_in_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                refreshGridOfToday();
                MessageBox.Show("Cash-In transaction added.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
